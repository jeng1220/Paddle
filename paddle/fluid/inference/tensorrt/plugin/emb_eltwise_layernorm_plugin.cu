...
template <typename T>
EmbEltwiseLayernormPluginDynamicImpl<T>::~EmbEltwiseLayernormPluginDynamicImpl() {
  this->terminate();
}
...
int EmbEltwiseLayernormPluginDynamicImpl<T>::enqueue(
  ...
  if (old_input_ptr_ != new_input_ptr) {
    ...
    cudaMemcpyAsync(in_ptr_gpu_d, reinterpret_cast<const void*>(inputs),
        sizeof(uintptr_t) * input_num, cudaMemcpyHostToDevice, stream);
  }
...
void EmbEltwiseLayernormPluginDynamic::terminate() {
...
nvinfer1::DataType EmbEltwiseLayernormPluginDynamic::getOutputDataType(
  ...
  PADDLE_ENFORCE_EQ(
      index, 0, platform::errors::InvalidArgument(
                    ...
                    index));
  if (with_fp16_) return nvinfer1::DataType::kHALF;
  else return nvinfer1::DataType::kFLOAT;
}
